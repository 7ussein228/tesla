...
router.get('/:id', async (req, res) => {
  try {
    ...
    const lectures = await db.prepare('SELECT * FROM lectures WHERE course_id = ? ORDER BY sort_order').all(req.params.id);
    const quizzes = await db.prepare('SELECT * FROM quizzes WHERE course_id = ?').all(req.params.id);
    const sheets = await db.prepare('SELECT * FROM sheets WHERE course_id = ?').all(req.params.id);
...
router.post('/:id/enroll', auth, async (req, res) => {
  try {
    const existing = await db.prepare('SELECT id FROM enrollments WHERE student_id = ? AND course_id = ?').get(req.user.id, req.params.id);
    if (existing) return res.status(409).json({ error: 'أنت مسجل بالفعل في هذا الكورس' });
    await db.prepare('INSERT INTO enrollments (student_id, course_id) VALUES (?, ?)').run(req.user.id, req.params.id);
...
router.post('/:id/lectures', auth, authorize('teacher', 'admin'), async (req, res) => {
  try {
    ...
    const lecture = await db.prepare('SELECT * FROM lectures WHERE id = ?').get(result.lastInsertRowid);
    res.json(lecture);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/lectures/:id/progress', auth, async (req, res) => {
  try {
    const { watched_seconds, completed } = req.body;
    const existing = await db.prepare('SELECT id FROM lecture_progress WHERE student_id = ? AND lecture_id = ?').get(req.user.id, req.params.id);
    if (existing) {
      await db.prepare(`
    ...
    const lecture = await db.prepare('SELECT course_id FROM lectures WHERE id = ?').get(req.params.id);
    if (lecture) {
      const total = await db.prepare('SELECT COUNT(*) as cnt FROM lectures WHERE course_id = ?').get(lecture.course_id);
      ...
      await db.prepare('UPDATE enrollments SET progress = ? WHERE student_id = ? AND course_id = ?').run(progress, req.user.id, lecture.course_id);
      // Add energy if completed
      if (completed) {
        await db.prepare('UPDATE users SET energy = energy + 50 WHERE id = ?').run(req.user.id);
      }
    }
